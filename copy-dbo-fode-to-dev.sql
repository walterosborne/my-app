/*
Copies every user table in dbo whose name begins with `Fode` or `fode`
into dev by creating new dev tables with SELECT INTO.
...
- It only includes tables whose names begin with `Fode` or `fode`.
- It excludes any source table whose name contains `Backup` or `_backup`.
...
WHERE source_schema.name = @SourceSchema
  AND LOWER(source_table.name) LIKE N'fode%'
...
BEGIN
    THROW 50070, 'Source and target schemas must be different.', 1;
END;
...
BEGIN
    THROW 50071, 'Source schema was not found. Check @SourceSchema before running this script.', 1;
END;
...
BEGIN
    THROW 50072, 'Target schema was not found. Check @TargetSchema before running this script.', 1;
END;
...
IF NOT EXISTS (
    ...
    WHERE s.name = @SourceSchema
      AND LOWER(t.name) LIKE N'fode%'
      AND LOWER(t.name) NOT LIKE N'%backup%'
)
BEGIN
    THROW 50073, 'No eligible Fode tables were found in the source schema.', 1;
END;
...
BEGIN
    ...
    THROW 50074, 'Refusing to create non-backup tables in dbo. Only _backup tables may be targeted in dbo.', 1;
END;
...
BEGIN TRY
    ...
    BEGIN
        ...
        BEGIN
            THROW 50075, 'A target table already exists in the target schema. Remove or rename it before rerunning.', 1;
        END;
