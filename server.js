...
const queueEmail = async (client, { toAddress, ccAddress = null, subject, body }) => {
    try {
...
const resolveApproverIdentifierToMyId = async (queryable, identifier) => {
    ...
    const rosterResult = await queryable.query(
        `SELECT myid
         FROM roster_r
         WHERE myid = $1
         LIMIT 1`,
    ...
    if (rosterResult.rows[0]?.myid) {
        return rosterResult.rows[0].myid;
    }
    ...
    try {
        ...
        if (ids) {
            ...
            result = await pool.query(
                `SELECT *
                 FROM roster_r
                 WHERE rostername ILIKE $1 OR myid ILIKE $1 OR email ILIKE $1 OR networkid ILIKE $1
                 ORDER BY CASE WHEN rostername ILIKE $2 THEN 0 ELSE 1 END, rostername
                 LIMIT $3`,
                [`%${trimmedQuery}%`, `${trimmedQuery}%`, parsedLimit]
            );
        } else {
            result = await pool.query('SELECT * FROM roster_r ORDER BY rosterName');
        }
        ...
        if (shouldSendScheduleEmail && scheduleIdToNotify && emailAuditorIds.size > 0) {
            const rosterResult = await client.query(
                `SELECT DISTINCT r.email
                 FROM auditors_r a
                 JOIN roster_r r ON r.myid = a.myid
                 WHERE a.auditorid = ANY($1) AND r.email IS NOT NULL`,
                [Array.from(emailAuditorIds)]
            );
            ...
            for (const approverId of missingIds) {
                await pool.query(
                    `INSERT INTO approvals_r (scheduleid, approvermyid, status, requestedat, approvedat, createdat, updatedat)
                     ...
                     ON CONFLICT (scheduleid, approvermyid) DO NOTHING`,
                    [
                        parseInt(scheduleId),
                        approverId,
...
app.post('/api/approvals/:scheduleId/remind', async (req, res) => {
    ...
    try {
        ...
        const approverIds = await resolveApproverIdentifiersToMyIds(client, audit.approver ? [audit.approver] : []);
        const additionalApproverIds = await resolveApproverIdentifiersToMyIds(client, audit.additionalApprovers);
        const requiredApproverIds = [...new Set([
            ...approverIds,
            ...(leadMyId ? [leadMyId] : []),
            ...additionalApproverIds
...
app.post('/api/unlock-audit', async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(
            "UPDATE audits_r SET locked = B'0', approvedat = NULL, submittedat = NULL WHERE scheduleid = $1",
            [parseInt(scheduleId)]
        );
                    ...
                    `UPDATE approvals_r
                     ...
                     WHERE scheduleid = $1
                       ...
                       AND NOT EXISTS (
                           SELECT 1
                           FROM approvals_r
                           WHERE scheduleid = $1 AND approvermyid = $3
                       )`,
                    [audit.scheduleId, identifier, resolvedMyId]
                );
                await client.query(
                    `DELETE FROM approvals_r
                     WHERE scheduleid = $1
                       AND approvermyid = $2
                       AND EXISTS (
                           SELECT 1
            ...
            for (const approverId of approvalMyIds) {
                await client.query(
                    `INSERT INTO approvals_r (scheduleid, approvermyid, status, requestedat, approvedat, createdat, updatedat)
                     ...
                     ON CONFLICT (scheduleid, approvermyid)
                     DO UPDATE SET status = 'pending', requestedat = CURRENT_TIMESTAMP, approvedat = NULL, updatedat = CURRENT_TIMESTAMP`,
